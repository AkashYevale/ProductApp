...
exports.createValidate = function () {
    return [
        ...
        body('START_DATE').isDate().notEmpty().withMessage('Start date should not be empty'),
...
exports.updateValidate = function () {
    return [
        ...
        body('START_DATE').isDate().notEmpty().withMessage('Start date should not be empty').optional(),
...
exports.get = (req, res) => {
    ...
    try {
        mm.executeDQL('select count(*) as cnt from ' + viewExamMaster + ' where 1 ' + countCriteria, [], supportKey, (error, results1) => {
            if (error) {
                console.log(error);
                res.send({
                    "code": 400,
