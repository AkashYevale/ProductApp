...
exports.createValidate = function () {
    return [
        ...
        body('FNAME').notEmpty().withMessage('Parent first name should not be empty'),
        body('LNAME').notEmpty().withMessage('Parent last name should not be empty'),
    ...
    try {
        mm.executeDQL('select count(*) as cnt from ' + viewParentMaster + ' where 1 ' + countCriteria, [], supportKey, (error, results1) => {
            if (error) {
                ...
                res.send({
                    "code": 400,
                    "message": "Failed to get parent count.",
                });
            }
            else {
